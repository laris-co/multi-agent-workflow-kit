...
> Tip: Allow direnv (`direnv allow`) to expose `maw-start`, `maw-setup`, `maw-agents`, `maw-kill`, `maw-send`, `maw-remove`, and `maw-uninstall` aliases that wrap the scripts in `.agents/scripts/`.
